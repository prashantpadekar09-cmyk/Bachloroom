...
export default function CreditPurchaseModal({ isOpen, onClose, onSuccess }: CreditPurchaseModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[2.5rem] bg-white shadow-2xl animate-in fade-in zoom-in duration-300">
        <button 
          onClick={onClose}
        ...
        <div className="p-8 md:p-12">
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-50 text-blue-600 text-sm font-bold mb-4">
                ...
                <div className="min-h-[400px]">
                  {paymentMode === 'razorpay' ? (
                    <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-slate-50 rounded-[2.5rem] border border-slate-100">
                      <div className="p-6 rounded-full bg-blue-100 text-blue-600 mb-6">
                      ...
                      <button
                        ...
                        disabled={loading}
                        className="w-full max-w-xs py-5 rounded-[2rem] bg-blue-600 text-white font-black text-lg shadow-xl shadow-blue-200 hover:bg-blue-700 transition active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3"
                      >
                        {loading ? (
              ...
              {PACKAGES.map((pkg) => (
                <div 
                  ...
                  className={`relative flex flex-col rounded-[2rem] border-2 transition-all duration-300 ${
                    pkg.popular 
                      ? "border-blue-500 bg-blue-50/30 shadow-xl shadow-blue-100 scale-105" 
                      : "border-gray-100 bg-white hover:border-blue-200"
                  } p-8`}
                >
                  {pkg.popular && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-blue-600 text-white text-xs font-black uppercase tracking-widest">
                      Best Value
                    </div>
                  )}

                  <div className="flex items-center justify-between mb-6">
                    <div className={`p-3 rounded-2xl ${pkg.popular ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600"}`}>
                  ...
                  <ul className="flex-grow space-y-4 mb-8">
                    {pkg.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-3 text-sm font-medium text-gray-600">
                        <div className={`p-1 rounded-full ${pkg.popular ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-400"}`}>
