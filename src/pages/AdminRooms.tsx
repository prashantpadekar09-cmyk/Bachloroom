...
export default function AdminRooms() {
  ...
  const openEditModal = (room: Room) => {
    ...
    setRoomForm({
      ...
      existingImages: parseStringArray(room.images),
      newImages: [],
  ...
  const handleEditSave = async (event: FormEvent) => {
    ...
    const amenities = roomForm.amenities
      ...
      .map((item) => item.trim())
      .filter(Boolean);
    const images = [...roomForm.existingImages, ...roomForm.newImages].filter(Boolean);
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="md:col-span-2">
                    <label className="mb-2 block text-sm font-semibold text-gray-700">
                      Room Photos <span className="text-xs font-normal text-gray-400">(PNG / JPG, high resolution)</span>
                    </label>
                    {roomForm.existingImages.length > 0 && (
                      <div className="mb-3 flex flex-wrap gap-2">
                        {roomForm.existingImages.map((src, idx) => (
                          <div key={idx} className="relative h-16 w-16 overflow-hidden rounded-lg border border-gray-200">
                            <img src={src} alt={`Image ${idx + 1}`} className="h-full w-full object-cover" loading="lazy" />
                            <button
                              type="button"
                              onClick={() => setRoomForm((prev) => ({ ...prev, existingImages: prev.existingImages.filter((_, i) => i !== idx) }))}
                              className="absolute right-0.5 top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-white text-[10px]"
                            >×</button>
                          </div>
                        ))}
                      </div>
                    )}
                    <label className="flex min-h-[96px] cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-blue-200 bg-blue-50/50 px-4 py-4 text-center transition hover:border-blue-400 hover:bg-blue-50">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                      <span className="text-sm font-semibold text-blue-700">Upload new photos</span>
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          ...
                          Promise.all(readers).then((results) => {
                            setRoomForm((prev) => ({
                              ...prev,
                              newImages: [...prev.newImages, ...results].slice(0, 5),
                            }));
                          });
                          event.target.value = "";
                        }}
                    ...
                    {roomForm.newImages.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {roomForm.newImages.map((src, idx) => (
                          <div key={idx} className="relative h-16 w-16 overflow-hidden rounded-lg border border-gray-200">
                            <img src={src} alt={`New ${idx + 1}`} className="h-full w-full object-cover" loading="lazy" />
                            <button
                              type="button"
                              onClick={() => setRoomForm((prev) => ({ ...prev, newImages: prev.newImages.filter((_, i) => i !== idx) }))}
                              className="absolute right-0.5 top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-white text-[10px]"
                            >×</button>
                          </div>
                        ))}
