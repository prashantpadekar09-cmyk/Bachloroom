...
// Dedicated luxury rooms endpoint — strictly isLuxury = 1, no fallback
router.get("/luxury", (req, res) => {
  try {
    const stmt = db.prepare("SELECT * FROM rooms WHERE isLuxury = 1 ORDER BY createdAt DESC");
    const rooms = stmt.all().map((r: any) => ({
      ...r,
      images: safeParseArray(r.images),
      amenities: safeParseArray(r.amenities),
    }));
...
router.get("/:id", (req, res) => {
  try {
    ...
    if (
      ...
      try {
        const result = awardReferralReward({
          refereeId: currentUser.id,
          type: "unlock_owner_contact",
          amount: REFERRAL_REWARD_USER_UNLOCK_RUPEES,
          roomId: room.id,
        });
        if (result.awarded) {
          scheduleSupabaseSync("referral unlock_owner_contact");
        }
      } catch (error) {
        console.warn("Referral reward skipped:", error);
      }
    }
    ...
    if (req.user.role === "owner" && wasFirstOwnerRoom) {
      try {
        awardReferralReward({
          ...
          type: "owner_listed_room",
          amount: REFERRAL_REWARD_OWNER_LIST_RUPEES,
          roomId: id,
        });
      } catch (error) {
        console.warn("Referral reward skipped:", error);
