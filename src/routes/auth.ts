...
router.post("/register", async (req, res) => {
  const { name, email, password, phone, role, aadhaarNumber, idDocument } = req.body;
  if (!name || !email || !password || !aadhaarNumber || !idDocument) {
  ...
  const normalizedAadhaarNumber = String(aadhaarNumber).replace(/\s+/g, "");
  if (!AADHAAR_REGEX.test(normalizedAadhaarNumber)) {
  ...
  try {
    ...
    const stmt = db.prepare(
      "INSERT INTO users (id, name, email, password, phone, role, aadhaarNumber, idDocument, isVerified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
    stmt.run(id, name, normalizedEmail, hashedPassword, phone || "", normalizedRole, normalizedAadhaarNumber, idDocument, 0);
...
router.post("/google", async (req, res) => {
  const { credential, mode, name, email, phone, role, aadhaarNumber, idDocument } = req.body;
  ...
  try {
    ...
    if (!aadhaarNumber || !idDocument) {
      return res.status(400).json({ error: "Aadhaar number and document are required for sign up" });
    }
    ...
    db.prepare(
      `
        INSERT INTO users (id, name, email, password, phone, role, aadhaarNumber, idDocument, isVerified, googleId)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `
    ).run(
      id,
...
router.get("/me", (req, res) => {
  ...
  try {
    ...
    const stmt = db.prepare(
      "SELECT id, name, email, phone, role, subscriptionPlan, isPremium, isVerified, idDocument, aadhaarNumber FROM users WHERE id = ?"
    );
