...
function generateUniqueReferralCode() {
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    const existing = db.prepare("SELECT id FROM users WHERE referralCode = ? LIMIT 1").get(candidate) as
      | { id: string }
      | undefined;
    if (!existing) {
      return candidate;
...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (normalizedReferrerCode) {
      const referrer = db
        .prepare("SELECT id FROM users WHERE referralCode = ? LIMIT 1")
        .get(normalizedReferrerCode) as { id: string } | undefined;
...
router.get("/me", (req, res) => {
  ...
  try {
    ...
    const stmt = db.prepare(
      "SELECT id, name, email, phone, role, subscriptionPlan, isPremium, isVerified, idDocument, aadhaarNumber, selfieImage, referralCode, referredById, referralBalance, referralEarnings FROM users WHERE id = ?"
    );
