...
export function awardReferralReward(params: {
  ...
  if (!referrerExists) {
    return { awarded: false, reason: "invalid-referrer" };
  }
  ...
  const run = db.transaction((): AwardResult => {
    ...
    db.prepare(
      `
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `
    ).run(transactionId, referrerId, params.refereeId, params.type, params.amount, params.roomId ?? null);

    db.prepare(
      `
        UPDATE users
        SET
          referralBalance = COALESCE(referralBalance, 0) + ?,
