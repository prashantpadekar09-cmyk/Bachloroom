...
export function createReferralWithdrawalRequest(params: { userId: string; amount: number; upiId: string }) {
  const run = db.transaction(() => {
    ...
    if (!Number.isFinite(params.amount) || params.amount <= 0) throw new Error("Invalid withdrawal amount");
    if (params.amount > balance) throw new Error("Insufficient referral balance");
...
export function listReferralWithdrawalsForAdmin(params?: { status?: string }) {
  ...
  return db.prepare(`
    SELECT
      w.id, w.userId, w.amount, w.upiId, w.status, w.adminNote,
      w.reviewedBy, w.reviewedAt, w.paidAt, w.createdAt,
...
export function updateReferralWithdrawalStatus(params: {
  ...
  const run = db.transaction(() => {
    ...
    if (withdrawal.status === params.status) return { updated: true as const };
