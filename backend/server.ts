...
import savedRoomRoutes from "./routes/saved_rooms.js";
...
async function startServer() {
  ...
  initSupabaseMirror()
    .then((state) => {
      if (state.enabled) {
        ...
        console.warn("[supabase] Mirror disabled:", state.error);
      }
    })
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    const distDir = path.resolve(FRONTEND_ROOT, "dist");
    app.use(express.static(distDir));
    app.get("*", (_req, res) => res.sendFile(path.resolve(distDir, "index.html")));
  }
